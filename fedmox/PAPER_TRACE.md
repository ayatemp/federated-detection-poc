...
| --- | --- |
| Freeze foundation-model backbone; train task head only | `PSSFLRunner` exchanges selected task-head state by default; `freeze_backbone` is provided for models with `backbone` module paths. |
...
| Spatial sparse MoE router for variable-size feature maps | `SpatialTop1MoE`: `Conv2d(C, K, 1)` + hard top-1 forward mask for `[B, C, H, W]`; straight-through backward keeps the router trainable. |
| Traditional top-1 ROI router for fixed-size ROI features | `Top1MoE`: `Linear(D, K)` + hard top-1 forward mask; straight-through backward keeps the router trainable. |
