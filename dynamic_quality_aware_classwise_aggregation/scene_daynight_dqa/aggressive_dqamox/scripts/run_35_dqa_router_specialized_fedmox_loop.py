#!/usr/bin/env python3
"""Run FedMoX-style learning loops with DQA-gated router specialization.

The loop keeps the same practical shape as the previous FedMoX-like runs:
client sampling, one local checkpoint per selected client, and a lightweight
LatentMoE head.  The new learning signal is explicit but not hard-coded blind:
each client gets a router target expert, and the strength of that target is
scaled by DQA pseudoGT quality/count/class composition for that round.
"""
...
DEFAULT_OUTPUT = AGG_ROOT / "output" / "35_dqa_router_specialized_fedmox_loop"
REPORTS_ROOT = AGG_ROOT / "reports"
SUMMARY_CSV = REPORTS_ROOT / "35_dqa_router_specialized_fedmox_loop_summary.csv"
FINAL_METRICS = "18_client_balanced_single_injection_dqamox_final_metrics.csv"
RUN_LABEL = "35"
RUN_DESCRIPTION = "DQA-gated router-specialized FedMoX loop"
RUN_PAPER_NOTES = "FedMoX spatial sparse routing/soft mixture; FedMoE-DA domain-aware aggregation; FedJETs class-subset specialists."
...
STAGES = [
    ...
    Stage("full20_30", phase1_rounds=20, phase2_rounds=30, min_best_map50=0.550, min_gain_vs_warmup=0.080),
]


TRIALS = [
    Trial(
        name="35a_hybrid_dqa4_moehead_router_specialists",
        hypothesis=(
            "DQA-gated class/domain router specialization.  If selected pseudoGT is VRU-heavy or traffic-heavy, "
            "the router is nudged to class experts; otherwise day/night domain experts are used.  The nudge is weak "
            "when pseudoGT quality/count is weak, so bad clients do not dominate."
        ),
        args=[
            "--num-experts", "4",
            "--top-k", "1",
            ...
            "--late-dqa-min-server-alpha", "0.30",
    ...
    Trial(
        name="35b_domain4_neckhead_soft_router_specialists",
        hypothesis=(
            "Domain/client specialist routing with slightly more representation movement.  Uses top-2 routing and neck/head "
            "updates so the router can form day/night and scene experts instead of only moving zero-initialized expert heads."
        ),
        args=[
            "--num-experts", "4",
            "--top-k", "2",
            ...
            "--router-specialization-weight", "0.075",
            "--router-specialization-max-weight", "0.055",
            ...
            "--late-expert-keep-fraction", "0.76",
            "--late-expert-max-class-fraction", "0.30",
            "--late-actual-max-class-fraction", "0.42",
            "--min-views", "2",
            "--min-models", "0",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--master-port", type=int, default=37801)
...
def run_stage(args: argparse.Namespace, trial: Trial, stage: Stage, trial_index: int) -> tuple[str, dict[str, Any]]:
    ...
    log_path = log_dir / f"{RUN_LABEL}_{trial.name}_{stage.name}_{started.strftime('%Y%m%d_%H%M%S')}.log"
    cmd = [
        ...
        "--client-sampling-seed", str(350000 + trial_index),
    ...
    _, warmup, best = read_metrics(workspace)
...
def format_stage_message(row: dict[str, Any]) -> str:
    return "\n".join(
        [
            f"{RUN_LABEL} stage finished: {row['trial']} / {row['stage']}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    intro = "\n".join(
        [
            f"{RUN_LABEL} started: {RUN_DESCRIPTION}.",
            f"- target mAP50={args.target_map50}",
            f"- papers used: {RUN_PAPER_NOTES}",
            "- setting: 50 FL rounds only if promising, 33.3% clients, one checkpoint per client, self-only pseudoGT.",
            f"- trials={', '.join(t.name for t in trials)}",
        ]
    )
    print(intro)
    if not args.no_discord:
        notify(intro, f"DQA-MoX {RUN_LABEL} started")
    ...
    for trial_index, trial in enumerate(trials):
        ...
        for stage in STAGES:
            ...
            print(message)
            if not args.no_discord:
                notify(message, f"DQA-MoX {RUN_LABEL} stage")
    ...
    outro = "\n".join(
        [
            f"{RUN_LABEL} loop finished without reaching target."
            if best
            else f"{RUN_LABEL} loop finished without valid metrics.",
    ...
    print(outro)
    if not args.no_discord:
        notify(outro, f"DQA-MoX {RUN_LABEL} result")
    return 2
