#!/usr/bin/env python3
"""Evaluate 27o asymmetric score-scaled routed MoE."""
...
DEFAULT_WORKSPACE = AGG_ROOT / "output" / "27o_asymmetric_score_routed_moe"
NOTEBOOK_PATH = AGG_ROOT / "notebooks" / "research_loop_until_060" / "009_27o_asymmetric_score_routed_moe.ipynb"
...
def write_spec(workspace: Path, label: str, groups: dict[str, list[Path]], group_scales: dict[str, list[float]] | None) -> Path:
    path = workspace / "stats" / "27o_routed_specs" / f"{label}.routed.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {
...
def make_candidates(workspace: Path) -> list[dict]:
    ...
    day_anchor = [1.10, 0.90, 1.00, 1.00, 0.50, 0.50, 0.50]
    day_light = [1.05, 0.90, 1.00, 1.00, 0.65, 0.65, 0.65]
    night_hard = [1.00, 1.00, 1.00, 1.00, 1.00, 1.00]
    night_precision = [1.00, 0.85, 0.95, 0.65, 0.65, 0.65]
    night_light = [1.05, 0.90, 1.00, 0.75, 0.75, 0.75]
    settings: list[tuple[str, dict[str, list[float]] | None, str]] = [
        ("hard_reference_t0.32", None, "27k hard routed reference"),
        ("day_anchor_night_hard", {"day": day_anchor, "night": night_hard}, "Keep 27n day gain but restore hard night route."),
        ("day_anchor_night_precision", {"day": day_anchor, "night": night_precision}, "Day anchor-dominant with precision-scaled night experts."),
        ("day_anchor_night_light", {"day": day_anchor, "night": night_light}, "Day anchor-dominant with lightly damped night clients."),
        ("day_light_night_hard", {"day": day_light, "night": night_hard}, "Less aggressive day client dampening with hard night route."),
    ]
    candidates = []
        ...
        writer.writerow(
            {
                ...
                "rationale": (
                    "27n found anchor-dominant score scaling improves total mAP but slightly hurts highway_night. "
                    "27o uses asymmetric score scaling to keep day gains while preserving night expert behavior."
                ),
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.no_discord:
        notify(
            "27o started: asymmetric score-scaled routed MoE. It tries day anchor gains with safer night routing.",
            "DQA-MoX 27o started",
        )
    ...
    for candidate in candidates:
        ...
        if row.get("status") == "ok" and row.get("map50") is not None and float(row["map50"]) >= args.target_map50:
            break
    total_csv = workspace / "stats" / "27o_asymmetric_score_total_metrics.csv"
    ...
    if not ok_rows:
        raise RuntimeError("No successful 27o total evaluations.")
    ...
    full_csv = workspace / "stats" / "27o_asymmetric_score_best_split_metrics.csv"
    ...
    message = "\n".join(
        [
            f"27o finished. Best total candidate: {best['candidate']}",
    ...
    print(message)
    if not args.no_discord:
        notify(message, "DQA-MoX 27o result")
