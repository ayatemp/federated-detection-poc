...
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "09_specialist_balanced_latent_dqamox.ipynb"
...
cells = [
    md(
        """
        # 09 Specialist-Balanced Latent DQA-MoX

        このノートブックは、08で見えた「latent MoEは崩壊を防ぐが、pseudoGTの偏りで伸び切らない」という問題に対する次ループです。
        ...
        - 評価は `warmup`, `warmup + server repair`, `warmup + specialist-balanced latent DQA-MoX + repair` を同じ scene/day-night protocol で比較する

        つまり、MoEの中身は匿名のまま保ちつつ、pseudoGT側だけを「expertが成立しやすい学習問題」に整えます。
        """
    ),
    code(
        """
        ...
        RUNNER = PROJECT_ROOT / "scripts" / "run_scene_daynight_dqa_09_specialist_balanced_latent_dqamox.py"
        WORKSPACE = PROJECT_ROOT / "output" / "09_specialist_balanced_latent_dqamox"
    ...
    code(
        ...
        design = pd.DataFrame([
            ...
            {
                "part": "Phase 1",
                "choice": "30 rounds, neck/head/router中心 + selected pseudoGT",
                "why": "pseudoGTのclass偏りを抑えて、rare/hard領域にも学習信号を回す",
            },
            {
                "part": "Phase 2",
            ...
            {
                "part": "comparison",
                "choice": "08 warmup / 08 warmup+repair / 09 DQA+repair",
    ...
    code(
        """
        spec = importlib.util.spec_from_file_location("dqa09_runner", RUNNER)
        ...
        estimate = pd.DataFrame([
            ...
            {"stage": "Phase1 specialist-balanced DQA", "count": args.phase1_rounds, "minutes_each": args.estimated_phase1_round_minutes},
    ...
    md(
        ...
        - `output/09_specialist_balanced_latent_dqamox/stats/09_specialist_balanced_latent_dqamox_final_metrics.csv`
        - `output/09_specialist_balanced_latent_dqamox/stats/09_specialist_balanced_latent_dqamox_split_metrics.csv`
        - `output/09_specialist_balanced_latent_dqamox/09_specialist_balanced_latent_dqamox_report.md`
    ...
    code(
        ...
        EXPERT_KEEP_FRACTION = 0.72
        EXPERT_MAX_CLASS_FRACTION = 0.30
        LOAD_BIAS_STRENGTH = 0.35
    ...
    code(
        """
        metrics_path = WORKSPACE / "stats" / "09_specialist_balanced_latent_dqamox_final_metrics.csv"
        split_path = WORKSPACE / "stats" / "09_specialist_balanced_latent_dqamox_split_metrics.csv"
        report_path = WORKSPACE / "09_specialist_balanced_latent_dqamox_report.md"
