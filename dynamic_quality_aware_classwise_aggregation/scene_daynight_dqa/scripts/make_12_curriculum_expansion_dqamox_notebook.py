...
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "12_curriculum_expansion_dqamox.ipynb"
...
cells = [
    md(
        """
        # 12 Curriculum-Expansion Full-From-Warmup DQA-MoX

        このノートブックは、warmupから最後まで一貫して学習する本実験です。
        08/09/10のようにwarmupやrepair baselineを再利用せず、FedMoX風にMoE headを最初から入れて学習します。
        ...
        - 後半は selected pseudoGT の採用率とpseudo比率を上げ、server anchorを弱めてtarget/domain信号を増やす
    ...
    code(
        ...
        RUNNER = PROJECT_ROOT / "scripts" / "run_scene_daynight_dqa_12_curriculum_expansion_dqamox.py"
        WORKSPACE = PROJECT_ROOT / "output" / "12_curriculum_expansion_dqamox"
    ...
    code(
        ...
        design = pd.DataFrame([
            ...
            {
                "part": "Phase 1",
                "choice": "30 rounds, early protective -> late expansion curriculum",
                "why": "11で見えたprotective plateauを越えるため、round9以降はpseudoGTを少し攻める",
            },
            {
                "part": "Phase 2",
            ...
            {
                "part": "aggregation",
                "choice": "DQA-CWA v2 with scheduled server anchor",
                "why": "前半はserverを強く残し、後半はclient/domain信号が集約に出るようにする",
            },
            {
                "part": "comparison",
    ...
    code(
        """
        spec = importlib.util.spec_from_file_location("dqa12_runner", RUNNER)
    ...
    md(
        ...
        - `output/12_curriculum_expansion_dqamox/stats/12_curriculum_expansion_dqamox_final_metrics.csv`
        - `output/12_curriculum_expansion_dqamox/stats/12_curriculum_expansion_dqamox_split_metrics.csv`
        - `output/12_curriculum_expansion_dqamox/12_curriculum_expansion_dqamox_report.md`
    ...
    code(
        ...
        ACTUAL_MAX_CLASS_FRACTION = 0.0
        LOAD_BIAS_STRENGTH = 0.45
        CURRICULUM_START_ROUND = 9
    ...
    code(
        """
        metrics_path = WORKSPACE / "stats" / "12_curriculum_expansion_dqamox_final_metrics.csv"
        split_path = WORKSPACE / "stats" / "12_curriculum_expansion_dqamox_split_metrics.csv"
        report_path = WORKSPACE / "12_curriculum_expansion_dqamox_report.md"
