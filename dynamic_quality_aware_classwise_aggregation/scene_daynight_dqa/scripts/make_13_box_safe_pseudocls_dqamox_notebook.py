...
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "13_box_safe_pseudocls_dqamox.ipynb"
...
cells = [
    md(
        """
        # 13 Box-Safe PseudoCls Full-From-Warmup DQA-MoX

        12の結果から、pseudoGTを増やしてbbox回帰まで学習させる方向は外れだと判断しました。
        このノートブックでは warmup から新規に学習しつつ、pseudoGTは主に class/objectness/router/domain 信号として使い、bbox回帰はほぼ source GT 側に任せます。
        ...
        - Phase1の pseudoGT bbox loss は 0.0 にして、pseudo bbox の局所化ノイズを学習しない
        - Phase2も bbox loss を 0.001 に抑え、最後だけごく弱く全体調整する
    ...
    code(
        ...
        RUNNER = PROJECT_ROOT / "scripts" / "run_scene_daynight_dqa_13_box_safe_pseudocls_dqamox.py"
        WORKSPACE = PROJECT_ROOT / "output" / "13_box_safe_pseudocls_dqamox"
    ...
    code(
        """
        spec = importlib.util.spec_from_file_location("dqa13_runner", RUNNER)
    ...
    md(
        ...
        - `output/13_box_safe_pseudocls_dqamox/stats/13_box_safe_pseudocls_dqamox_final_metrics.csv`
        - `output/13_box_safe_pseudocls_dqamox/stats/13_box_safe_pseudocls_dqamox_split_metrics.csv`
        - `output/13_box_safe_pseudocls_dqamox/13_box_safe_pseudocls_dqamox_report.md`
    ...
    code(
        ...
        EXPERT_MAX_CLASS_FRACTION = 0.22
        ACTUAL_MAX_CLASS_FRACTION = 0.28
    ...
    code(
        """
        metrics_path = WORKSPACE / "stats" / "13_box_safe_pseudocls_dqamox_final_metrics.csv"
        split_path = WORKSPACE / "stats" / "13_box_safe_pseudocls_dqamox_split_metrics.csv"
        report_path = WORKSPACE / "13_box_safe_pseudocls_dqamox_report.md"
