#!/usr/bin/env python3
"""Run a FedMoX-style full-from-warmup DQA-MoX experiment.
...
The DQA branch keeps the strongest lesson from 09/10: raw pseudoGT is not used
as-is. Pseudo boxes are routed into expert-choice buckets, head-class dominance
is capped, and client training remains source-dominant so pseudo localization
noise does not become the main optimization target.
"""
...
PROTOCOL_VERSION = "scene_daynight_dqa_11_fedmox_full_from_warmup_target_v1"
...
def write_warmup_config(setup, fedsto, args: argparse.Namespace, weights: Path) -> Path:
    cfg = setup.efficientteacher_config(
        name="sdn11_fedmox_full_from_warmup_target_warmup",
    ...
    return setup.write_config("sdn11_fedmox_full_from_warmup_target_warmup.yaml", cfg)
...
def write_client_config(
    ...
    *,
    ...
    run_name = f"sdn11_{condition}_p{phase}_{tag}_{client_tag}"
...
def write_server_repair_config(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    run_name = f"sdn11_{condition}_p{phase}_{tag}_server_repair"
    cfg = setup.efficientteacher_config(
        name=run_name,
        train=setup.LIST_ROOT / "server_cloudy_train.txt",
        val=setup.LIST_ROOT / "server_cloudy_val.txt",
...
def attach_selected_box_tables(args: argparse.Namespace, pseudo_stats: dict[str, Any], round_idx: int) -> dict[str, Any]:
    ...
    for client_tag, stats in pseudo_stats.get("clients", {}).items():
        ...
        if not selected:
            continue
        box_table = args.workspace_root / "stats" / f"11_{tag}_{client_tag}_specialist_selected_boxes.csv"
...
def run_server_repair_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    repair_start = fedsto.GLOBAL_DIR / f"sdn11_{condition}_p{phase}_{tag}_server_repair_start.pt"
...
def run_repair_baseline(
    ...
    *,
    ...
    for idx in range(1, args.repair_baseline_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "11_repair_baseline_checkpoints.csv", records)
...
def run_dqa_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    print(f"\n=== fedmox_full_from_warmup_target phase {phase} {tag}: pseudo labels ===")
    ...
    for client in clients:
        client_tag = f"client{client['id']}_{client['weather']}"
        start = fedsto.CLIENT_STATE_DIR / f"sdn11_p{phase}_{tag}_{client_tag}_start.pt"
    ...
    state_path = args.workspace_root / "stats" / "11_latent_dqamox_dqa_state.json"
    if not args.dry_run and not reusable_checkpoint(fedsto, aggregate, args):
        ...
        (args.workspace_root / "stats" / f"11_p{phase}_{tag}_dqa_state_snapshot.json").write_text(
...
def write_final_metrics(args: argparse.Namespace, eval_records: list[dict[str, str]]) -> list[dict[str, Any]]:
        ...
        metric_rows.append(
            {
                ...
                "condition": {
                    ...
                    "latent_dqamox_final_aggregate": "warmup + FedMoX-style full-from-warmup DQA-MoX aggregate",
                    "latent_dqamox_final_repair": "warmup + FedMoX-style full-from-warmup DQA-MoX + server repair",
                }.get(label, label),
    ...
    write_csv(args.workspace_root / "stats" / "11_fedmox_full_from_warmup_target_final_metrics.csv", metric_rows, fields)
    ...
    write_csv(
        args.workspace_root / "stats" / "11_fedmox_full_from_warmup_target_split_metrics.csv",
        split_rows,
...
def write_report(args: argparse.Namespace, metrics: list[dict[str, Any]], run_manifest: dict[str, Any]) -> Path:
    path = args.workspace_root / "11_fedmox_full_from_warmup_target_report.md"
    lines = [
        "# 11 FedMoX-Style Full-From-Warmup DQA-MoX Report",
        "",
...
def progress_factory(args: argparse.Namespace, total: int):
    ...
    try:
        ...
        return None
    return tqdm(total=total, desc="11 FedMoX-style full-from-warmup DQA-MoX", unit="step")
...
def run(args: argparse.Namespace) -> None:
    ...
    (args.workspace_root / "stats" / "11_fedmox_full_from_warmup_target_manifest.json").write_text(
    ...
    def maybe_notify_progress(stage: str, completed: int, checkpoint: Path | None = None) -> None:
        ...
        notify(
            args,
            f"DQA 11 progress: {stage}, ETA {seconds_to_hms(eta)}.",
            title="DQA 11 progress",
            status="running",
            extra_context=extra,
        )
    ...
    for idx in range(1, args.phase1_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "11_latent_dqamox_checkpoints.csv", dqa_records)
        if progress is not None:
            elapsed = time.monotonic() - start_time
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "11_all_checkpoints.csv", records)
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "11_eval_checkpoints.csv", eval_records)
    ...
    (args.workspace_root / "stats" / "11_fedmox_full_from_warmup_target_run_manifest.json").write_text(
...
def notify(
    ...
    *,
    ...
    try:
        ...
        metrics_path = args.workspace_root.expanduser().resolve() / "stats" / "11_fedmox_full_from_warmup_target_final_metrics.csv"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--workspace-root", type=Path, default=PROJECT_ROOT / "output" / "11_fedmox_full_from_warmup_target")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if do_start_notify:
        notify(args, "Scene-Daynight DQA 11 FedMoX-style full-from-warmup DQA-MoX started.", title="DQA 11 start")
    ...
    try:
        ...
        if do_end_notify:
            notify(
                args,
                f"Scene-Daynight DQA 11 FedMoX-style full-from-warmup DQA-MoX finished with status={status}.",
                title="DQA 11 finish",
                status=status,
                error=error,
            )
