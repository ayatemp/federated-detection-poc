#!/usr/bin/env python3
"""Run a box-safe pseudo-class full-from-warmup DQA-MoX experiment.
...
This is the follow-up loop after 12. 12 showed that increasing pseudoGT box
exposure and weakening the server anchor did not lift mAP; it continued the
slow drift down. 13 keeps the same FedMoX-style full-from-warmup protocol but
changes the role of pseudoGT:
...
PROTOCOL_VERSION = "scene_daynight_dqa_13_box_safe_pseudocls_dqamox_v1"
...
def write_warmup_config(setup, fedsto, args: argparse.Namespace, weights: Path) -> Path:
    cfg = setup.efficientteacher_config(
        name="sdn13_box_safe_pseudocls_dqamox_warmup",
    ...
    return setup.write_config("sdn13_box_safe_pseudocls_dqamox_warmup.yaml", cfg)
...
def write_client_config(
    ...
    *,
    ...
    run_name = f"sdn13_{condition}_p{phase}_{tag}_{client_tag}"
...
def write_server_repair_config(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    run_name = f"sdn13_{condition}_p{phase}_{tag}_server_repair"
    cfg = setup.efficientteacher_config(
        name=run_name,
        train=setup.LIST_ROOT / "server_cloudy_train.txt",
        val=setup.LIST_ROOT / "server_cloudy_val.txt",
...
def attach_selected_box_tables(args: argparse.Namespace, pseudo_stats: dict[str, Any], round_idx: int) -> dict[str, Any]:
    ...
    for client_tag, stats in pseudo_stats.get("clients", {}).items():
        ...
        if not selected:
            continue
        box_table = args.workspace_root / "stats" / f"13_{tag}_{client_tag}_specialist_selected_boxes.csv"
...
def rebalance_selected_box_tables(
    ...
    for client_tag, client_rows in sorted(by_client.items()):
        ...
        box_table = args.workspace_root / "stats" / f"13_{tag}_{client_tag}_actual_class_balanced_boxes.csv"
        write_csv(
            box_table,
            selected_rows,
            [
    ...
    write_csv(
        args.workspace_root / "stats" / f"13_{tag}_actual_class_rebalance_stats.csv",
        summary_rows,
        [
            "round",
            "client",
    ...
    write_csv(
        args.workspace_root / "stats" / f"13_{tag}_actual_class_rebalance_boxes.csv",
        all_selected_rows,
        [
            "round",
            "client",
...
def run_server_repair_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    repair_start = fedsto.GLOBAL_DIR / f"sdn13_{condition}_p{phase}_{tag}_server_repair_start.pt"
...
def run_repair_baseline(
    ...
    *,
    ...
    for idx in range(1, args.repair_baseline_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "13_repair_baseline_checkpoints.csv", records)
...
def run_dqa_round(
    ...
    *,
    ...
    print(f"\n=== box_safe_pseudocls_dqamox phase {phase} {tag}: pseudo labels ===")
    ...
    for client in clients:
        client_tag = f"client{client['id']}_{client['weather']}"
        start = fedsto.CLIENT_STATE_DIR / f"sdn13_p{phase}_{tag}_{client_tag}_start.pt"
    ...
    state_path = args.workspace_root / "stats" / "13_latent_dqamox_dqa_state.json"
    if not round_args.dry_run and not reusable_checkpoint(fedsto, aggregate, round_args):
        ...
        (args.workspace_root / "stats" / f"13_p{phase}_{tag}_dqa_state_snapshot.json").write_text(
...
def write_final_metrics(args: argparse.Namespace, eval_records: list[dict[str, str]]) -> list[dict[str, Any]]:
        ...
        metric_rows.append(
            {
                ...
                "condition": {
                    ...
                    "latent_dqamox_final_aggregate": "warmup + Box-safe pseudo-class full-from-warmup DQA-MoX aggregate",
                    "latent_dqamox_final_repair": "warmup + Box-safe pseudo-class full-from-warmup DQA-MoX + server repair",
                }.get(label, label),
    ...
    write_csv(args.workspace_root / "stats" / "13_box_safe_pseudocls_dqamox_final_metrics.csv", metric_rows, fields)
    ...
    write_csv(
        args.workspace_root / "stats" / "13_box_safe_pseudocls_dqamox_split_metrics.csv",
        split_rows,
...
def write_report(args: argparse.Namespace, metrics: list[dict[str, Any]], run_manifest: dict[str, Any]) -> Path:
    path = args.workspace_root / "13_box_safe_pseudocls_dqamox_report.md"
    lines = [
        "# 13 Box-Safe PseudoCls Full-From-Warmup DQA-MoX Report",
        "",
...
def progress_factory(args: argparse.Namespace, total: int):
    ...
    try:
        ...
        return None
    return tqdm(total=total, desc="13 Box-safe pseudo-class full-from-warmup DQA-MoX", unit="step")
...
def run(args: argparse.Namespace) -> None:
    ...
    (args.workspace_root / "stats" / "13_box_safe_pseudocls_dqamox_manifest.json").write_text(
    ...
    def maybe_notify_progress(stage: str, completed: int, checkpoint: Path | None = None) -> None:
        ...
        notify(
            args,
            f"DQA 13 progress: {stage}, ETA {seconds_to_hms(eta)}.",
            title="DQA 13 progress",
            status="running",
            extra_context=extra,
        )
    ...
    for idx in range(1, args.phase1_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "13_latent_dqamox_checkpoints.csv", dqa_records)
        if progress is not None:
            elapsed = time.monotonic() - start_time
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "13_all_checkpoints.csv", records)
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "13_eval_checkpoints.csv", eval_records)
    ...
    (args.workspace_root / "stats" / "13_box_safe_pseudocls_dqamox_run_manifest.json").write_text(
...
def notify(
    ...
    *,
    ...
    try:
        ...
        metrics_path = args.workspace_root.expanduser().resolve() / "stats" / "13_box_safe_pseudocls_dqamox_final_metrics.csv"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--workspace-root", type=Path, default=PROJECT_ROOT / "output" / "13_box_safe_pseudocls_dqamox")
    ...
    parser.add_argument("--master-port", type=int, default=36481)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if do_start_notify:
        notify(args, "Scene-Daynight DQA 13 Box-safe pseudo-class full-from-warmup DQA-MoX started.", title="DQA 13 start")
    ...
    try:
        ...
        if do_end_notify:
            notify(
                args,
                f"Scene-Daynight DQA 13 Box-safe pseudo-class full-from-warmup DQA-MoX finished with status={status}.",
                title="DQA 13 finish",
                status=status,
                error=error,
            )
