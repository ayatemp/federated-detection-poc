#!/usr/bin/env python3
"""Run full warmup-to-final latent DQA-MoX.

This is the first end-to-end MoE-DQA runner: the detector is born with an
anonymous latent MoE head during warmup and keeps that architecture through
repair-only and DQA branches.  Experts are not tied to hand-written meanings,
clients, scenes, or classes; a learned spatial router chooses them.
"""
...
PROTOCOL_VERSION = "scene_daynight_dqa_08_full_latent_dqamox_v1"
...
def write_warmup_config(setup, fedsto, args: argparse.Namespace, weights: Path) -> Path:
    cfg = setup.efficientteacher_config(
        name="sdn08_latent_dqamox_warmup",
    ...
    return setup.write_config("sdn08_latent_dqamox_warmup.yaml", cfg)
...
def write_client_config(
    ...
    *,
    ...
    run_name = f"sdn08_{condition}_p{phase}_{tag}_{client_tag}"
...
def write_server_repair_config(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    run_name = f"sdn08_{condition}_p{phase}_{tag}_server_repair"
    cfg = setup.efficientteacher_config(
        name=run_name,
        train=setup.LIST_ROOT / "server_cloudy_train.txt",
        val=setup.LIST_ROOT / "server_cloudy_val.txt",
...
def run_server_repair_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    repair_start = fedsto.GLOBAL_DIR / f"sdn08_{condition}_p{phase}_{tag}_server_repair_start.pt"
...
def run_repair_baseline(
    ...
    *,
    ...
    for idx in range(1, args.repair_baseline_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "08_repair_baseline_checkpoints.csv", records)
...
def run_dqa_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    print(f"\n=== latent_dqamox phase {phase} {tag}: pseudo labels ===")
    pseudo_stats = pl03.generate_round_pseudo_labels(setup, current, args, clients, round_idx)
    records: list[dict[str, str]] = []
    local_paths: list[Path] = []

    for client in clients:
        client_tag = f"client{client['id']}_{client['weather']}"
        start = fedsto.CLIENT_STATE_DIR / f"sdn08_p{phase}_{tag}_{client_tag}_start.pt"
    ...
    state_path = args.workspace_root / "stats" / "08_latent_dqamox_dqa_state.json"
    if not args.dry_run and not reusable_checkpoint(fedsto, aggregate, args):
        ...
        (args.workspace_root / "stats" / f"08_p{phase}_{tag}_dqa_state_snapshot.json").write_text(
...
def write_final_metrics(args: argparse.Namespace, eval_records: list[dict[str, str]]) -> list[dict[str, Any]]:
        ...
        metric_rows.append(
            {
                ...
                "condition": {
                    ...
                    "latent_dqamox_final_aggregate": "warmup + latent DQA-MoX aggregate",
                    "latent_dqamox_final_repair": "warmup + latent DQA-MoX + server repair",
                }.get(label, label),
    ...
    write_csv(args.workspace_root / "stats" / "08_full_latent_dqamox_final_metrics.csv", metric_rows, fields)
    ...
    write_csv(
        args.workspace_root / "stats" / "08_full_latent_dqamox_split_metrics.csv",
        split_rows,
...
def write_report(args: argparse.Namespace, metrics: list[dict[str, Any]], run_manifest: dict[str, Any]) -> Path:
    path = args.workspace_root / "08_full_latent_dqamox_report.md"
    lines = [
        "# 08 Full Latent DQA-MoX Report",
        "",
    ...
    lines.extend(
        [
            ...
            "",
            "- This run is not client-id MoE and not hand-labeled expert MoE. Expert identities are latent and learned by the router.",
            "- DQA remains in the pseudoGT statistics and classwise server-anchored aggregation; MoE remains inside the detector head from warmup onward.",
...
def progress_factory(args: argparse.Namespace, total: int):
    ...
    try:
        ...
        return None
    return tqdm(total=total, desc="08 full latent DQA-MoX", unit="step")
...
def run(args: argparse.Namespace) -> None:
    ...
    (args.workspace_root / "stats" / "08_full_latent_dqamox_manifest.json").write_text(
    ...
    def maybe_notify_progress(stage: str, completed: int, checkpoint: Path | None = None) -> None:
        ...
        notify(
            args,
            f"DQA 08 progress: {stage}, ETA {seconds_to_hms(eta)}.",
            title="DQA 08 progress",
            status="running",
            extra_context=extra,
        )
    ...
    for idx in range(1, args.phase1_rounds + 1):
        round_records, dqa_current, pseudo_stats, port_offset = run_dqa_round(
            setup,
            fedsto,
            dqa_current,
            args,
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "08_latent_dqamox_checkpoints.csv", dqa_records)
        if progress is not None:
            elapsed = time.monotonic() - start_time
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "08_all_checkpoints.csv", records)
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "08_eval_checkpoints.csv", eval_records)
    ...
    (args.workspace_root / "stats" / "08_full_latent_dqamox_run_manifest.json").write_text(
...
def notify(
    ...
    *,
    ...
    try:
        ...
        metrics_path = args.workspace_root.expanduser().resolve() / "stats" / "08_full_latent_dqamox_final_metrics.csv"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--workspace-root", type=Path, default=PROJECT_ROOT / "output" / "08_full_latent_dqamox_from_warmup")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if do_start_notify:
        notify(args, "Scene-Daynight DQA 08 full latent DQA-MoX started.", title="DQA 08 start")
    ...
    try:
        ...
        if do_end_notify:
            notify(
                args,
                f"Scene-Daynight DQA 08 full latent DQA-MoX finished with status={status}.",
                title="DQA 08 finish",
                status=status,
                error=error,
            )
