#!/usr/bin/env python3
"""Create notebook 35: DQA-gated router-specialized FedMoX loop."""
...
NOTEBOOK = PROJECT_ROOT / "notebooks" / "35_dqa_router_specialized_fedmox_loop.ipynb"
    ...
    notebook = {
        "cells": [
            markdown_cell(
                ...
                "Learning-first self-only loop.  The detector keeps a LatentMoE head, but router/expert specialization is now explicit: "
                "DQA pseudoGT quality, selected-box count, and class composition decide how strongly each client/domain/class should pull the router toward a specialist expert. "
                "The controller uses FedMoX-like client sampling and stops early if the result does not clear warmup by enough."
            ),
