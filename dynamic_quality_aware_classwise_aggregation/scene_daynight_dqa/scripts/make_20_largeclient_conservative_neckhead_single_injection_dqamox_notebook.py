...
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "20_largeclient_conservative_neckhead_single_injection_dqamox.ipynb"
...
cells = [
    md(
        """
        # 20 Large-Client Conservative Neck/Head Single-Injection DQA-MoX

        15では pseudo label path 問題を直し、YOLO が生成済み pseudo label を読むことを確認しました。
        その結果、round1/2では warmup を少し超えましたが、pseudoGT round を長く続けると再び性能が落ちました。

        このノートブックでは、FedMoX 風に warmup から MoE head を含めて一貫して学習しつつ、
        DQA/pseudoGT は短く注入し、その後は pseudoGT を増やさず source server repair で固めます。

        重要な設計は次の通りです。

        - warmupを `LatentMoEYoloV5` で最初から学習する
        - warmup+repair baseline は既存比較を参照し、このノートブックでは DQA branch を中心に回す
        - DQA branch は Phase1を短くして、pseudoGT self-training drift を避ける
        - DQA後に source consolidation repair を複数round入れ、MoE headを壊さず局所化を戻す
        ...
        - Phase2はデフォルトでは使わず、必要なら後続ループで追加する
        - クライアント学習だけでなく DQA の classwise stats も selected pseudoGT から作る
        - DQA stats では client total を median に正規化し、pseudoGT が少ない night/rare client が集計で埋もれないようにする
    ...
    code(
        ...
        RUNNER = PROJECT_ROOT / "scripts" / "run_scene_daynight_dqa_20_largeclient_conservative_neckhead_single_injection_dqamox.py"
        WORKSPACE = PROJECT_ROOT / "output" / "20_largeclient_conservative_neckhead_single_injection_dqamox"
    ...
    code(
        ...
        design = pd.DataFrame([
            ...
            {
                "part": "Phase 1",
                "choice": "2 rounds, strict pseudoGT + weak localization",
                "why": "15では早いroundだけ上がったため、pseudoGT注入を短くする",
            },
            {
                "part": "post-DQA consolidation",
                "choice": "6 rounds, source server repair, MoE head retained",
                "why": "pseudoGTを増やさず、早いDQA gainをsource局所化で固める",
            },
            {
                "part": "aggregation",
                "choice": "DQA-CWA v2 with stable server anchor + client-balanced stats",
                "why": "15ではnight/rare clientのselected pseudoGTが少なく、raw countではday/citystreetがDQA集計を支配しやすい",
            },
            {
                "part": "comparison",
    ...
    code(
        """
        spec = importlib.util.spec_from_file_location("dqa18_runner", RUNNER)
        ...
        estimate = pd.DataFrame([
            ...
            {"stage": "post-DQA source consolidation", "count": args.post_dqa_repair_rounds, "minutes_each": args.estimated_repair_round_minutes},
    ...
    md(
        ...
        - `output/20_largeclient_conservative_neckhead_single_injection_dqamox/stats/20_largeclient_conservative_neckhead_single_injection_dqamox_final_metrics.csv`
        - `output/20_largeclient_conservative_neckhead_single_injection_dqamox/stats/20_largeclient_conservative_neckhead_single_injection_dqamox_split_metrics.csv`
        - `output/20_largeclient_conservative_neckhead_single_injection_dqamox/20_largeclient_conservative_neckhead_single_injection_dqamox_report.md`
    ...
    code(
        ...
        cmd = [
            ...
            "--post-dqa-repair-rounds", str(POST_DQA_REPAIR_ROUNDS),
            ...
            "--notify",
            "--notify-progress",
            "--notify-first-progress-hours", "1",
            "--notify-progress-interval-hours", "1",
        ]
        print(" ".join(cmd))
        if RUN_FULL:
            subprocess.run(cmd, cwd=PROJECT_ROOT, check=True)
    ...
    code(
        """
        metrics_path = WORKSPACE / "stats" / "20_largeclient_conservative_neckhead_single_injection_dqamox_final_metrics.csv"
        split_path = WORKSPACE / "stats" / "20_largeclient_conservative_neckhead_single_injection_dqamox_split_metrics.csv"
        report_path = WORKSPACE / "20_largeclient_conservative_neckhead_single_injection_dqamox_report.md"
