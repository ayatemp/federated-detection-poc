#!/usr/bin/env python3
"""Run specialist-balanced latent DQA-MoX from an existing warmup.

09 is the first follow-up loop after 08. It keeps the anonymous latent MoE
head, but changes the pseudo-GT learning problem before client training:
...
* train clients on source GT + the selected pseudoGT only;
...
reused from 08 so this loop can focus on improving the DQA branch.
"""
...
PROTOCOL_VERSION = "scene_daynight_dqa_09_specialist_balanced_latent_dqamox_v1"
...
def write_warmup_config(setup, fedsto, args: argparse.Namespace, weights: Path) -> Path:
    cfg = setup.efficientteacher_config(
        name="sdn09_specialist_balanced_latent_dqamox_warmup",
    ...
    return setup.write_config("sdn09_specialist_balanced_latent_dqamox_warmup.yaml", cfg)
...
def write_client_config(
    ...
    *,
    ...
    run_name = f"sdn09_{condition}_p{phase}_{tag}_{client_tag}"
...
def write_server_repair_config(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    run_name = f"sdn09_{condition}_p{phase}_{tag}_server_repair"
    cfg = setup.efficientteacher_config(
        name=run_name,
        train=setup.LIST_ROOT / "server_cloudy_train.txt",
        val=setup.LIST_ROOT / "server_cloudy_val.txt",
...
def attach_selected_box_tables(args: argparse.Namespace, pseudo_stats: dict[str, Any], round_idx: int) -> dict[str, Any]:
    ...
    for client_tag, stats in pseudo_stats.get("clients", {}).items():
        ...
        if not selected:
            continue
        box_table = args.workspace_root / "stats" / f"09_{tag}_{client_tag}_specialist_selected_boxes.csv"
...
def run_server_repair_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    repair_start = fedsto.GLOBAL_DIR / f"sdn09_{condition}_p{phase}_{tag}_server_repair_start.pt"
...
def run_repair_baseline(
    ...
    *,
    ...
    for idx in range(1, args.repair_baseline_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "09_repair_baseline_checkpoints.csv", records)
...
def run_dqa_round(
    ...
    *,
    ...
    tag = round_tag(round_idx)
    print(f"\n=== specialist_balanced_latent_dqamox phase {phase} {tag}: pseudo labels ===")
    ...
    for client in clients:
        client_tag = f"client{client['id']}_{client['weather']}"
        start = fedsto.CLIENT_STATE_DIR / f"sdn09_p{phase}_{tag}_{client_tag}_start.pt"
    ...
    state_path = args.workspace_root / "stats" / "09_latent_dqamox_dqa_state.json"
    if not args.dry_run and not reusable_checkpoint(fedsto, aggregate, args):
        ...
        (args.workspace_root / "stats" / f"09_p{phase}_{tag}_dqa_state_snapshot.json").write_text(
...
def write_final_metrics(args: argparse.Namespace, eval_records: list[dict[str, str]]) -> list[dict[str, Any]]:
        ...
        metric_rows.append(
            {
                ...
                "condition": {
                    ...
                    "latent_dqamox_final_aggregate": "warmup + specialist-balanced latent DQA-MoX aggregate",
                    "latent_dqamox_final_repair": "warmup + specialist-balanced latent DQA-MoX + server repair",
                }.get(label, label),
    ...
    write_csv(args.workspace_root / "stats" / "09_specialist_balanced_latent_dqamox_final_metrics.csv", metric_rows, fields)
    ...
    write_csv(
        args.workspace_root / "stats" / "09_specialist_balanced_latent_dqamox_split_metrics.csv",
        split_rows,
...
def write_report(args: argparse.Namespace, metrics: list[dict[str, Any]], run_manifest: dict[str, Any]) -> Path:
    path = args.workspace_root / "09_specialist_balanced_latent_dqamox_report.md"
    lines = [
        "# 09 Specialist-Balanced Latent DQA-MoX Report",
        "",
...
def progress_factory(args: argparse.Namespace, total: int):
    ...
    try:
        ...
        return None
    return tqdm(total=total, desc="09 specialist-balanced latent DQA-MoX", unit="step")
...
def run(args: argparse.Namespace) -> None:
    ...
    payload = {
        ...
        "pseudo_selection": {
            "method": "expert_choice_specialist_balanced",
    ...
    (args.workspace_root / "stats" / "09_specialist_balanced_latent_dqamox_manifest.json").write_text(
    ...
    def maybe_notify_progress(stage: str, completed: int, checkpoint: Path | None = None) -> None:
        ...
        notify(
            args,
            f"DQA 09 progress: {stage}, ETA {seconds_to_hms(eta)}.",
            title="DQA 09 progress",
            status="running",
            extra_context=extra,
        )
    ...
    for idx in range(1, args.phase1_rounds + 1):
        ...
        write_checkpoint_records(args.workspace_root / "stats" / "09_latent_dqamox_checkpoints.csv", dqa_records)
        if progress is not None:
            elapsed = time.monotonic() - start_time
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "09_all_checkpoints.csv", records)
    ...
    write_checkpoint_records(args.workspace_root / "stats" / "09_eval_checkpoints.csv", eval_records)
    ...
    (args.workspace_root / "stats" / "09_specialist_balanced_latent_dqamox_run_manifest.json").write_text(
...
def notify(
    ...
    *,
    ...
    try:
        ...
        metrics_path = args.workspace_root.expanduser().resolve() / "stats" / "09_specialist_balanced_latent_dqamox_final_metrics.csv"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--workspace-root", type=Path, default=PROJECT_ROOT / "output" / "09_specialist_balanced_latent_dqamox")
    ...
    parser.add_argument("--phase1-source-repeat", type=int, default=2)
    parser.add_argument("--phase1-pseudo-repeat", type=int, default=2)
    parser.add_argument("--phase1-loss-box", type=float, default=0.003)
    ...
    parser.add_argument("--phase2-loss-box", type=float, default=0.006)
    ...
    parser.add_argument("--dqa-server-anchor", type=float, default=0.45)
    parser.add_argument("--dqa-min-server-alpha", type=float, default=0.45)
    parser.add_argument("--dqa-residual-blend", type=float, default=0.25)
    parser.add_argument("--expert-count", type=int, default=4)
    parser.add_argument("--expert-keep-fraction", type=float, default=0.72)
    parser.add_argument("--expert-max-class-fraction", type=float, default=0.30)
    parser.add_argument("--load-bias-strength", type=float, default=0.35)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if do_start_notify:
        notify(args, "Scene-Daynight DQA 09 specialist-balanced latent DQA-MoX started.", title="DQA 09 start")
    ...
    try:
        ...
        if do_end_notify:
            notify(
                args,
                f"Scene-Daynight DQA 09 specialist-balanced latent DQA-MoX finished with status={status}.",
                title="DQA 09 finish",
                status=status,
                error=error,
            )
