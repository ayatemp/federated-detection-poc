#!/usr/bin/env python3
"""Run scene DQA-CWA with a learned adaptive pseudoGT threshold policy.

This wrapper is for notebook 07.  It keeps the 06 scene/class schedule, but
...
The EfficientTeacher patch currently reads DQA06_* environment variable names,
so this runner uses DQA07_* for its own settings and exports the compatible
DQA06_* names only for the child train process.
"""
...
DQA_PROTOCOL_VERSION = "dqa07_scene_learned_adaptive_policy_v1"
...
def _profile_name() -> str:
    return os.environ.get("DQA07_SSOD_PROFILE", "learned_adaptive_policy")


def _stats_root() -> Path | None:
    raw = os.environ.get("DQA07_STATS_ROOT")
    return Path(raw).resolve() if raw else None


def _threshold_log() -> Path | None:
    raw = os.environ.get("DQA07_THRESHOLD_LOG")
    if raw:
        return Path(raw).resolve()
    stats_root = _stats_root()
    return stats_root / "learned_adaptive_policy_schedule.jsonl" if stats_root else None


def _policy_model_path() -> Path:
    raw = os.environ.get("DQA07_POLICY_MODEL")
    if raw:
        return Path(raw).resolve()
    return (
        base.RESEARCH_ROOT
...
def _base_decision(phase: int | None, round_idx: int | None, client_id: str | None, reason: str) -> ThresholdDecision:
    nc = _int_env("DQA07_NUM_CLASSES", 10)
...
def _smooth_decision(decision: ThresholdDecision) -> ThresholdDecision:
    if not _bool_env("DQA07_ENABLE_SMOOTHING", True):
        return decision
    ...
    low_step = _float_env("DQA07_LOW_STEP_LIMIT", 0.02)
    high_step = _float_env("DQA07_HIGH_STEP_LIMIT", 0.035)
    nms_step = _float_env("DQA07_NMS_STEP_LIMIT", 0.02)
    loss_step = _float_env("DQA07_LOSS_STEP_LIMIT", 0.015)
...
def _latest_policy_rows(stats_root: Path, source_round: int, client_id: str):
    ...
    horizon = max(_int_env("DQA07_POLICY_HORIZON_ROUNDS", _int_env("DQA07_PHASE2_ROUNDS", 24)), source_round)
...
def _decision_from_policy(
    *,
    ...
    min_low = _float_env("DQA07_MIN_LOW", 0.35)
    max_low = _float_env("DQA07_MAX_LOW", 0.46)
    min_high = _float_env("DQA07_MIN_HIGH", 0.75)
    max_high = _float_env("DQA07_MAX_HIGH", 0.87)
    min_high_gap = _float_env("DQA07_MIN_HIGH_GAP", 0.34)
    min_nms = _float_env("DQA07_MIN_NMS", 0.35)
    max_nms = _float_env("DQA07_MAX_NMS", 0.42)
    rare_count = _float_env("DQA07_RARE_COUNT", 250.0)
    rare_max_low = _float_env("DQA07_RARE_MAX_LOW", 0.42)
    rare_max_high = _float_env("DQA07_RARE_MAX_HIGH", 0.84)
    ...
    for row in rows.to_dict("records"):
        ...
        low = _clamp(float(row["pred_low"]), min_low, max_low)
        high = _clamp(float(row["pred_high"]), min_high, max_high)
        if count < rare_count:
            low = min(low, rare_max_low)
            high = min(high, rare_max_high)
        high = _clamp(max(high, low + min_high_gap), min_high, max_high)
    ...
    teacher = _clamp(
        0.35 - _float_env("DQA07_TEACHER_DROP", 0.05) * risk,
        _float_env("DQA07_TEACHER_MIN", 0.30),
        0.35,
    )
    box = _clamp(0.02 - 0.003 * risk, 0.016, 0.02)
    obj = _clamp(0.35 - 0.04 * risk, 0.30, 0.35)
    cls = _clamp(0.10 - 0.025 * risk, 0.07, 0.10)
...
def decide_thresholds(name: str, num_classes: int) -> ThresholdDecision:
    ...
    start_round = _int_env("DQA07_ADAPT_START_ROUND", 3)
    stats_root = _stats_root()
...
def _prepare_learned_scene_modules(work_root: Path):
    setup, fedsto = _prepare_scene_modules(work_root)
    exact_setup = setup.base
    original_config = getattr(
        exact_setup,
        "_dqa07_original_efficientteacher_config",
        exact_setup.efficientteacher_config,
    )
    exact_setup._dqa07_original_efficientteacher_config = original_config
    client_lr0 = _float_env("DQA07_CLIENT_LR0", 3e-4)
    server_lr0 = _float_env("DQA07_SERVER_LR0", 1e-3)

    def learned_config(**kwargs):
    ...
    setup.efficientteacher_config = learned_config
    exact_setup.efficientteacher_config = learned_config
    fedsto.setup.efficientteacher_config = learned_config
    return setup, fedsto
...
def _learned_run_train(fedsto, config: Path, args, *, extra_env: dict[str, str] | None = None):
    ...
    if "client" in config.name:
        ...
        decision = decide_thresholds(run_name, _int_env("DQA07_NUM_CLASSES", 10))
...
base.DEFAULT_DQA_WORK_ROOT = base.RESEARCH_ROOT / "efficientteacher_dqa07_scene_learned_adaptive_policy_8h"
base._prepare_fedsto_modules = _prepare_learned_scene_modules
base.run_train = _learned_run_train


def main() -> None:
    parsed = base.parse_args()
    os.environ["DQA07_STATS_ROOT"] = str(parsed.stats_root.resolve())
    os.environ["DQA07_PHASE2_ROUNDS"] = str(parsed.phase2_rounds)
    os.environ["DQA07_NUM_CLASSES"] = str(parsed.num_classes)
    os.environ.setdefault("DQA07_POLICY_HORIZON_ROUNDS", str(parsed.phase2_rounds))
    os.environ.setdefault(
        "DQA07_THRESHOLD_LOG",
        str((parsed.stats_root / "learned_adaptive_policy_schedule.jsonl").resolve()),
    )
    ...
    if parsed.setup_only:
        ...
        print(f"DQA07 profile: {_profile_name()}")
        print(f"DQA07 policy model: {_policy_model_path()}")
