#!/usr/bin/env python3
"""Run warm-up -> Phase 1 -> Phase 2 with stable DQA-SBA.

08_4_2 keeps the 08_4 idea: DQA starts in Phase 1 and controls only a
server-anchored backbone residual.  Phase 2 is changed into a plateau-style
refinement: target clients are less allowed to move the already adapted model,
and DQA-CWA residuals decay as rounds progress.
"""
...
DQA_PROTOCOL_VERSION = "dqa08_4_2_scene_phase1_dqa_sba_stable_policy_v1"
DEFAULT_WORK_ROOT = base.RESEARCH_ROOT / "efficientteacher_dqa08_4_2_scene_phase1_dqa_sba_stable_policy"
...
def _scheduled(prefix: str, round_idx: int | None, default_start: float, default_end: float) -> float:
    ...
    total = max(_int_env("DQA0842_PHASE2_ROUNDS", 12), 1)
...
def _phase2_config(config: v2.AggregationConfig, round_idx: int | None) -> v2.AggregationConfig:
    classwise_blend = _scheduled_value(
        "DQA0842_PHASE2_CLASSWISE_BLEND",
        round_idx,
        min(float(config.classwise_blend), 0.055),
        0.015,
    )
    residual_blend = _scheduled_value("DQA0842_PHASE2_RESIDUAL", round_idx, 0.055, 0.010)
    min_server_alpha = _scheduled_value("DQA0842_PHASE2_MIN_SERVER_ALPHA", round_idx, 0.78, 0.92)
    server_anchor = _scheduled_value(
        "DQA0842_PHASE2_SERVER_ANCHOR",
        round_idx,
        max(float(config.server_anchor), 14.0),
        24.0,
    )
    temperature = _scheduled_value(
        "DQA0842_PHASE2_TEMPERATURE",
        round_idx,
        max(float(config.temperature), 2.8),
        3.4,
    )
    stability_lambda = _scheduled_value(
        "DQA0842_PHASE2_STABILITY_LAMBDA",
        round_idx,
        max(float(config.stability_lambda), 0.65),
        0.82,
    )
    return replace(
        copy.copy(config),
...
def _aggregate_checkpoints(
    ...
    if phase == 1:
        print(
            "DQA08_4_2 Phase 1 aggregation: DQA-SBA "
    ...
    record = {
        ...
        "phase2_client_train_scope": os.environ.get("DQA0842_PHASE2_CLIENT_TRAIN_SCOPE", "neck_head"),
        "phase2_pseudo_loss_scale": _scheduled_value("DQA0842_PHASE2_PSEUDO_LOSS_SCALE", round_idx, 0.85, 0.55),
    }
...
def _aggregate_fedavg_checkpoints(
    ...
    *,
    ...
    if phase == 1:
        print("DQA08_4_2 Phase 1 fallback: conservative backbone residual average")
    ...
    print("DQA08_4_2 Phase 2 fallback: scheduled server-anchored residual average")
    ...
    def stable_config(**kwargs):
        ...
        if target is not None and phase == 2:
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get(
                "DQA0842_PHASE2_CLIENT_TRAIN_SCOPE",
                "neck_head",
            )
            cfg.setdefault("hyp", {})["lr0"] = _scheduled_value("DQA0842_PHASE2_CLIENT_LR0", round_idx, 2.5e-4, 1.0e-4)
            cfg["hyp"]["lrf"] = 1.0
            scale = _scheduled_value("DQA0842_PHASE2_PSEUDO_LOSS_SCALE", round_idx, 0.85, 0.55)
            ...
            ssod["ignore_obj"] = os.environ.get("DQA0842_PHASE2_IGNORE_OBJ", "1").strip().lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0842_PHASE2_SERVER_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0842_PHASE2_SERVER_ORTHOGONAL_WEIGHT", 1e-4)
            cfg.setdefault("hyp", {})["lr0"] = _scheduled_value("DQA0842_PHASE2_SERVER_LR0", round_idx, 8.0e-4, 4.0e-4)
            cfg["hyp"]["lrf"] = 1.0
...
def _stable_run_train(fedsto, config: Path, args, *, extra_env: dict[str, str] | None = None):
    ...
    if parsed and parsed["role"].startswith("client") and int(parsed["phase"]) == 2:
        env.setdefault("ET_MAX_PSEUDO_PER_IMAGE", os.environ.get("DQA0842_MAX_PSEUDO_PER_IMAGE", "80"))
        env.setdefault("ET_MAX_PSEUDO_PER_CLASS_IMAGE", os.environ.get("DQA0842_MAX_PSEUDO_PER_CLASS_IMAGE", "25"))
...
def main() -> None:
    ...
    if parsed.dqa_start_phase != 1:
        print(f"DQA08_4_2 forces --dqa-start-phase 1; got {parsed.dqa_start_phase}, overriding.")
    ...
    os.environ["DQA0842_PHASE2_ROUNDS"] = str(parsed.phase2_rounds)
    os.environ["DQA08_NUM_CLASSES"] = str(parsed.num_classes)
    os.environ["DQA084_PHASE1_ROUNDS"] = str(parsed.phase1_rounds)
    os.environ.setdefault("DQA084_PHASE1_RESIDUAL_START", "0.45")
    os.environ.setdefault("DQA084_PHASE1_RESIDUAL_END", "0.06")
    os.environ.setdefault("DQA084_PHASE1_MAX_RELATIVE_UPDATE", "0.03")
    os.environ.setdefault("DQA08_POLICY_HORIZON_ROUNDS", str(max(parsed.phase2_rounds, 1)))
    os.environ.setdefault(
        "DQA08_THRESHOLD_LOG",
        str((parsed.stats_root / "phase1_dqa_sba_stable_policy_schedule.jsonl").resolve()),
    )
    ...
    if parsed.setup_only:
        ...
        print("DQA08_4_2 profile: phase1 DQA-SBA + stable phase2 plateau")
        print(f"DQA08_4_2 protocol: {DQA_PROTOCOL_VERSION}")
        print(
            "DQA08_4_2 phase1 residual schedule:",
            os.environ.get("DQA084_PHASE1_RESIDUAL_START", "0.45"),
            "->",
            os.environ.get("DQA084_PHASE1_RESIDUAL_END", "0.06"),
        )
        print(
            "DQA08_4_2 phase2 residual schedule:",
            os.environ.get("DQA0842_PHASE2_RESIDUAL_START", "0.055"),
            "->",
            os.environ.get("DQA0842_PHASE2_RESIDUAL_END", "0.010"),
        )
        print(f"DQA08_4_2 phase2 client train_scope: {os.environ.get('DQA0842_PHASE2_CLIENT_TRAIN_SCOPE', 'neck_head')}")
        print(f"DQA08_4_2 policy model: {tri_stage._policy_model_path()}")
