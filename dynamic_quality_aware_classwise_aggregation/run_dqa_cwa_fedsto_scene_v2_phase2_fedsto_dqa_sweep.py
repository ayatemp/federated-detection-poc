#!/usr/bin/env python3
"""Run DQA08_3_2 phase2-only FedSTO/DQA comparison sweeps.

This runner keeps the useful 08_3 setup, but makes phase2 deliberately
comparative:

- one variant can run FedSTO-style full-parameter phase2 with plain FedAvg
- DQA variants can keep full-parameter client updates while controlling how much
  of those updates enter the global model
- DQA variants can switch pseudo-label thresholds/losses without editing the
  EfficientTeacher tree for every experiment
"""
...
DQA_PROTOCOL_VERSION = "dqa08_3_2_scene_phase2_fedsto_dqa_sweep_v1"
DEFAULT_WORK_ROOT = base.RESEARCH_ROOT / "efficientteacher_dqa08_3_2_phase2_fedsto_dqa_sweep"
...
def _profile() -> str:
    return os.environ.get("DQA0832_PROFILE", "dqa_high_light").strip().lower()


def _source_workspace() -> Path:
    return Path(os.environ.get("DQA0832_SOURCE_WORK_ROOT", str(DEFAULT_SOURCE_WORK_ROOT))).resolve()


def _source_phase1_round() -> int:
    return _int_env("DQA0832_SOURCE_PHASE1_ROUND", DEFAULT_SOURCE_PHASE1_ROUND)
...
def _scheduled(prefix: str, round_idx: int | None, default_start: float, default_end: float | None = None) -> float:
    ...
    total = max(_int_env("DQA0832_PHASE2_ROUNDS", 10), 1)
...
def _seed_phase1_checkpoint(workspace_root: Path) -> Path:
    ...
    force = _bool_env("DQA0832_FORCE_SEED", False)

    if not source.exists():
        raise FileNotFoundError(
            f"Missing source phase1 seed checkpoint: {source}\n"
            "Set DQA0832_SOURCE_WORK_ROOT / DQA0832_SOURCE_PHASE1_ROUND for a different seed."
        )
    ...
    wanted: dict[str, Any] = {
        ...
        "variant": os.environ.get("DQA0832_VARIANT", "unnamed"),
        "profile": _profile(),
    }
    ...
    if target.exists() and not force:
        if metadata_path.exists():
            ...
            if same_seed:
                print(f"Reusing 08_3_2 phase2 seed checkpoint: {target}")
                return target
        raise RuntimeError(
            ...
            "Use DQA0832_FORCE_SEED=1 only if you intentionally want to overwrite it."
        )

    target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, target)
    metadata_path.write_text(json.dumps(wanted, indent=2), encoding="utf-8")
    print(f"Seeded 08_3_2 phase2 from DQA08 phase1 round {source_round}: {target}")
    return target


def _override_if_env(cfg: dict[str, Any], key: str, env_name: str) -> None:
...
def _apply_dqa_ssod_profile(cfg: dict[str, Any], *, name: str, target: Path | None) -> None:
    ...
    decision = tri_stage.decide_thresholds(name, int(cfg["Dataset"]["nc"]))
    ...
    ssod["ignore_obj"] = _bool_env("DQA0832_IGNORE_OBJ", False)
    ssod["pseudo_label_with_obj"] = True
    ssod["pseudo_label_with_bbox"] = _bool_env("DQA0832_PSEUDO_LABEL_WITH_BBOX", True)
    ssod["pseudo_label_with_cls"] = _bool_env("DQA0832_PSEUDO_LABEL_WITH_CLS", True)

    _override_if_env(ssod, "teacher_loss_weight", "DQA0832_TEACHER_LOSS_WEIGHT")
    _override_if_env(ssod, "box_loss_weight", "DQA0832_BOX_LOSS_WEIGHT")
    _override_if_env(ssod, "obj_loss_weight", "DQA0832_OBJ_LOSS_WEIGHT")
    _override_if_env(ssod, "cls_loss_weight", "DQA0832_CLS_LOSS_WEIGHT")
...
def _prepare_phase2_sweep_modules(work_root: Path):
    ...
    original_config = getattr(
        exact_setup,
        "_dqa0832_original_efficientteacher_config",
        exact_setup.efficientteacher_config,
    )
    exact_setup._dqa0832_original_efficientteacher_config = original_config
    ...
    def sweep_config(**kwargs):
        ...
        if target is not None:
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0832_CLIENT_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0832_CLIENT_ORTHOGONAL_WEIGHT", 1e-4)
            cfg.setdefault("hyp", {})["lr0"] = _float_env("DQA0832_CLIENT_LR0", cfg.get("hyp", {}).get("lr0", 0.01))
            cfg["hyp"]["lrf"] = 1.0
            if profile != "fedsto_exact":
                _apply_dqa_ssod_profile(cfg, name=name, target=target)
        elif name != "runtime_server_warmup":
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0832_SERVER_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0832_SERVER_ORTHOGONAL_WEIGHT", 1e-4)
            cfg.setdefault("hyp", {})["lr0"] = _float_env("DQA0832_SERVER_LR0", cfg.get("hyp", {}).get("lr0", 0.01))
            cfg["hyp"]["lrf"] = 1.0
...
def _sweep_run_train(fedsto, config: Path, args, *, extra_env: dict[str, str] | None = None):
    ...
    decision = tri_stage.decide_thresholds(run_name, _int_env("DQA08_NUM_CLASSES", 10))
    if decision.phase is not None and decision.phase >= 2 and decision.client_id is not None:
        ...
        env["DQA08_TRI_STAGE_GATE"] = "1" if _bool_env("DQA0832_TRI_STAGE_GATE", True) else "0"
        env["DQA08_IGNORE_THRES_MID"] = json.dumps(decision.ignore_thres_mid)
        env["DQA08_LOW_MID_OBJ_WEIGHT"] = str(_float_env("DQA0832_LOW_MID_OBJ_WEIGHT", 0.35))
        env["DQA08_MID_HIGH_OBJ_WEIGHT"] = str(_float_env("DQA0832_MID_HIGH_OBJ_WEIGHT", 0.90))
...
def _dqa_config(args) -> v2.AggregationConfig:
    return v2.AggregationConfig(
        ...
        min_server_alpha=_float_env("DQA0832_MIN_SERVER_ALPHA", 0.65),
        residual_blend=_float_env("DQA0832_RESIDUAL_START", 0.16),
    )
...
def _aggregate_phase2_sweep(
    ...
    residual = _scheduled("DQA0832_RESIDUAL", round_idx, 0.16, 0.08)
    min_server_alpha = _scheduled(
        "DQA0832_MIN_SERVER_ALPHA",
        round_idx,
        _float_env("DQA0832_MIN_SERVER_ALPHA_START", _float_env("DQA0832_MIN_SERVER_ALPHA", 0.65)),
        _float_env("DQA0832_MIN_SERVER_ALPHA_END", _float_env("DQA0832_MIN_SERVER_ALPHA", 0.65)),
    )
    cfg = replace(
        copy.copy(config),
    ...
    state.setdefault("phase2_fedsto_dqa_sweep", []).append(
        {
            "round": round_idx,
            "variant": os.environ.get("DQA0832_VARIANT", "unnamed"),
            "profile": _profile(),
    ...
    print(
        "DQA08_3_2 phase2 gate:",
        f"variant={os.environ.get('DQA0832_VARIANT', 'unnamed')}",
        f"profile={_profile()}",
        f"round={round_idx}",
...
def main() -> None:
    ...
    os.environ["DQA0832_PHASE2_ROUNDS"] = str(parsed.phase2_rounds)
    ...
    os.environ.setdefault(
        "DQA08_THRESHOLD_LOG",
        str((parsed.stats_root / "phase2_fedsto_dqa_policy_schedule.jsonl").resolve()),
    )
    ...
    if parsed.phase1_rounds != 0:
        raise ValueError("08_3_2 is phase2-only. Run with --phase1-rounds 0.")
    ...
    print("DQA08_3_2 profile: phase2-only FedSTO/DQA sweep")
    print(f"DQA08_3_2 variant: {os.environ.get('DQA0832_VARIANT', 'unnamed')}")
    print(f"DQA08_3_2 profile mode: {_profile()}")
    print(f"DQA08_3_2 seed: {_source_workspace()} phase1 round {_source_phase1_round()}")
    print(f"DQA08_3_2 client train_scope: {os.environ.get('DQA0832_CLIENT_TRAIN_SCOPE', 'all')}")
    print(
        "DQA08_3_2 residual schedule:",
        _float_env("DQA0832_RESIDUAL_START", 0.16),
        "->",
        _float_env("DQA0832_RESIDUAL_END", 0.08),
    )
    print(f"DQA08_3_2 dqa_start_phase: {parsed.dqa_start_phase}")
