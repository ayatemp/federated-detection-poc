#!/usr/bin/env python3
"""Run DQA08_3_7 phase2 with R-SCoLQ-gated pseudo boxes.
...
DQA_PROTOCOL_VERSION = "dqa08_3_7_scene_phase2_rscolq_policy_v1"
DEFAULT_WORK_ROOT = base.RESEARCH_ROOT / "efficientteacher_dqa08_3_7_phase2_rscolq_policy"
...
def _source_workspace() -> Path:
    return Path(os.environ.get("DQA0837_SOURCE_WORK_ROOT", str(DEFAULT_SOURCE_WORK_ROOT))).resolve()


def _source_phase1_round() -> int:
    return _int_env("DQA0837_SOURCE_PHASE1_ROUND", DEFAULT_SOURCE_PHASE1_ROUND)


def _rscolq_model_path() -> Path:
    return Path(os.environ.get("DQA0837_RSCOLQ_MODEL", str(DEFAULT_RSCOLQ_MODEL))).resolve()
...
def _scheduled(prefix: str, round_idx: int | None, default_start: float, default_end: float | None = None) -> float:
    ...
    total = max(_int_env("DQA0837_PHASE2_ROUNDS", 10), 1)
...
def _rscolq_thresholds(nc: int) -> tuple[list[float], list[float], list[float]]:
    low = _float_env("DQA0837_RSCOLQ_LOW", 0.10)
    mid = _float_env("DQA0837_RSCOLQ_MID", 0.30)
    high = _float_env("DQA0837_RSCOLQ_HIGH", 0.60)
    lows = _parse_float_list(os.environ.get("DQA0837_CLASSWISE_LOW"), nc) or [low] * nc
    mids = _parse_float_list(os.environ.get("DQA0837_CLASSWISE_MID"), nc) or [mid] * nc
    highs = _parse_float_list(os.environ.get("DQA0837_CLASSWISE_HIGH"), nc) or [high] * nc
    out_low: list[float] = []
    out_mid: list[float] = []
    out_high: list[float] = []
...
def _rscolq_round_context(round_idx: int | None, nc: int) -> dict[str, Any]:
    ...
    if anchor_total <= 0.0:
        neutral.update({"previous_round": previous_round, "previous_stats": summary, "reason": "missing_anchor"})
        return neutral

    global_budget_margin = float(policy.get("global_budget_margin", 0.01))
    quality_margin = float(policy.get("quality_margin", 0.005))
    global_power = float(policy.get("global_power", 3.0))
    quality_power = float(policy.get("quality_power", 10.0))
    class_power = float(policy.get("class_power", 0.35))
    min_multiplier = float(policy.get("min_multiplier", 0.15))
    ...
    global_multiplier = math.exp(-global_power * pseudo_inflation) * math.exp(-quality_power * quality_inflation)

    class_multipliers: list[float] = []
    for idx in range(nc):
        anchor_count = max(float(anchor_counts_raw.get(str(idx), 1.0)), 1.0)
        ratio = float(summary["class_counts"][idx]) / (anchor_count * (1.0 + global_budget_margin))
        if ratio > 1.0:
    ...
    return {
        ...
        "global_multiplier": round(min(max(global_multiplier, min_multiplier), 1.0), 6),
...
def _log_rscolq_context(run_name: str, context: dict[str, Any]) -> None:
    ...
    record = {"run_name": run_name, "variant": os.environ.get("DQA0837_VARIANT", "unnamed"), **context}
...
def _seed_phase1_checkpoint(workspace_root: Path) -> Path:
    ...
    force = _bool_env("DQA0837_FORCE_SEED", False)

    if not source.exists():
        raise FileNotFoundError(
            f"Missing source phase1 seed checkpoint: {source}\n"
            "Set DQA0837_SOURCE_WORK_ROOT / DQA0837_SOURCE_PHASE1_ROUND for a different seed."
        )
    ...
    wanted: dict[str, Any] = {
        ...
        "variant": os.environ.get("DQA0837_VARIANT", "unnamed"),
    ...
    if target.exists() and not force:
        if metadata_path.exists():
            ...
            if same_seed:
                print(f"Reusing 08_3_7 phase2 seed checkpoint: {target}")
                return target
        raise RuntimeError(
            ...
            "Use DQA0837_FORCE_SEED=1 only if you intentionally want to overwrite it."
        )

    target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, target)
    metadata_path.write_text(json.dumps(wanted, indent=2), encoding="utf-8")
    print(f"Seeded 08_3_7 phase2 from DQA08 phase1 round {source_round}: {target}")
    return target
...
def _apply_rscolq_ssod_profile(cfg: dict[str, Any], *, name: str, target: Path | None) -> None:
    ...
    ssod["nms_conf_thres"] = _scheduled_value("DQA0837_NMS_CONF_THRES", round_idx, 0.01)
    ssod["ignore_thres_low"] = min(lows)
    ssod["ignore_thres_high"] = min(highs)
    ssod["teacher_loss_weight"] = _scheduled_value("DQA0837_TEACHER_LOSS_WEIGHT", round_idx, 0.32)
    ssod["box_loss_weight"] = _scheduled_value("DQA0837_BOX_LOSS_WEIGHT", round_idx, 0.010)
    ssod["obj_loss_weight"] = _scheduled_value("DQA0837_OBJ_LOSS_WEIGHT", round_idx, 0.32)
    ssod["cls_loss_weight"] = _scheduled_value("DQA0837_CLS_LOSS_WEIGHT", round_idx, 0.08)
    ssod["ignore_obj"] = _bool_env("DQA0837_IGNORE_OBJ", False)
...
def _prepare_rscolq_modules(work_root: Path):
    ...
    def rscolq_config(**kwargs):
        ...
        if target is not None:
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0837_CLIENT_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0837_CLIENT_ORTHOGONAL_WEIGHT", 1e-4)
            ...
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0837_SERVER_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0837_SERVER_ORTHOGONAL_WEIGHT", 1e-4)
            cfg.setdefault("hyp", {})["lr0"] = _scheduled_value(
                "DQA0837_SERVER_LR0",
                round_idx,
                cfg.get("hyp", {}).get("lr0", 0.01),
...
def _rscolq_run_train(fedsto, config: Path, args, *, extra_env: dict[str, str] | None = None):
    ...
    env.setdefault("ET_MAX_PSEUDO_PER_IMAGE", os.environ.get("DQA0837_MAX_PSEUDO_PER_IMAGE", "80"))
    env.setdefault("ET_MAX_PSEUDO_PER_CLASS_IMAGE", os.environ.get("DQA0837_MAX_PSEUDO_PER_CLASS_IMAGE", "25"))
    ...
    if parsed and parsed["role"].startswith("client") and parsed["phase"] >= 2:
        ...
        env["DQA0837_RSCOLQ_ENABLE"] = "1"
        env["DQA0837_RSCOLQ_MODEL"] = str(_rscolq_model_path())
        env["DQA0837_RSCOLQ_SCORE_POWER"] = os.environ.get("DQA0837_RSCOLQ_SCORE_POWER", "1.0")
        env["DQA0837_RSCOLQ_GLOBAL_MULTIPLIER"] = str(context["global_multiplier"])
        env["DQA0837_RSCOLQ_CLASS_MULTIPLIERS"] = json.dumps(context["class_multipliers"])
        env["DQA0836_SCOLQ_ENABLE"] = "1"
        env["DQA0836_SCOLQ_MODEL"] = str(_rscolq_model_path())
        env["DQA0836_SCOLQ_SCORE_POWER"] = os.environ.get("DQA0837_RSCOLQ_SCORE_POWER", "1.0")
        env["DQA06_NMS_CONF_THRES"] = str(_float_env("DQA0837_NMS_CONF_THRES", 0.01))
        ...
        env["DQA08_LOW_MID_OBJ_WEIGHT"] = str(_float_env("DQA0837_LOW_MID_OBJ_WEIGHT", 0.25))
        env["DQA08_MID_HIGH_OBJ_WEIGHT"] = str(_float_env("DQA0837_MID_HIGH_OBJ_WEIGHT", 0.70))
        _log_rscolq_context(run_name, context)
        print(
            "DQA08_3_7 R-SCoLQ gate:",
...
def _dqa_config(args) -> v2.AggregationConfig:
    return v2.AggregationConfig(
        ...
        min_server_alpha=_float_env("DQA0837_MIN_SERVER_ALPHA", 0.70),
        residual_blend=_float_env("DQA0837_RESIDUAL_START", 0.14),
    )
...
def _aggregate_rscolq(
    ...
    residual = _scheduled("DQA0837_RESIDUAL", round_idx, 0.14, 0.06)
    min_server_alpha = _scheduled(
        "DQA0837_MIN_SERVER_ALPHA",
        round_idx,
        _float_env("DQA0837_MIN_SERVER_ALPHA_START", _float_env("DQA0837_MIN_SERVER_ALPHA", 0.70)),
        _float_env("DQA0837_MIN_SERVER_ALPHA_END", _float_env("DQA0837_MIN_SERVER_ALPHA", 0.74)),
    )
    cfg = replace(
        copy.copy(config),
    ...
    state.setdefault("phase2_rscolq_policy", []).append(
        {
            "round": round_idx,
            "variant": os.environ.get("DQA0837_VARIANT", "unnamed"),
            "rscolq_model": str(_rscolq_model_path()),
    ...
    print(
        "DQA08_3_7 aggregate:",
        f"variant={os.environ.get('DQA0837_VARIANT', 'unnamed')}",
        f"round={round_idx}",
...
def main() -> None:
    ...
    os.environ["DQA0837_PHASE2_ROUNDS"] = str(parsed.phase2_rounds)
    ...
    os.environ.setdefault(
        "DQA08_THRESHOLD_LOG",
        str((parsed.stats_root / "phase2_rscolq_policy_schedule.jsonl").resolve()),
    )
    ...
    if parsed.phase1_rounds != 0:
        raise ValueError("08_3_7 is phase2-only. Run with --phase1-rounds 0.")
    ...
    print("DQA08_3_7 profile: phase2-only R-SCoLQ-gated pseudo boxes")
    print(f"DQA08_3_7 variant: {os.environ.get('DQA0837_VARIANT', 'unnamed')}")
    print(f"DQA08_3_7 seed: {_source_workspace()} phase1 round {_source_phase1_round()}")
    print(f"DQA08_3_7 R-SCoLQ model: {_rscolq_model_path()}")
    print(f"DQA08_3_7 thresholds: low={lows} mid={mids} high={highs}")
    print(f"DQA08_3_7 client train_scope: {os.environ.get('DQA0837_CLIENT_TRAIN_SCOPE', 'all')}")
    print(
        "DQA08_3_7 residual schedule:",
        _float_env("DQA0837_RESIDUAL_START", 0.14),
        "->",
        _float_env("DQA0837_RESIDUAL_END", 0.06),
    )
    print(
        "DQA08_3_7 pseudo loss:",
        f"teacher={_float_env('DQA0837_TEACHER_LOSS_WEIGHT', 0.32)}",
        f"box={_float_env('DQA0837_BOX_LOSS_WEIGHT', 0.010)}",
        f"obj={_float_env('DQA0837_OBJ_LOSS_WEIGHT', 0.32)}",
        f"cls={_float_env('DQA0837_CLS_LOSS_WEIGHT', 0.08)}",
    )
    print(f"DQA08_3_7 dqa_start_phase: {parsed.dqa_start_phase}")
