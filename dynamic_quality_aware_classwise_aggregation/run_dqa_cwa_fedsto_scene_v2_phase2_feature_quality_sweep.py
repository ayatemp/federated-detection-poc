#!/usr/bin/env python3
"""Run DQA08_3_4 phase2 feature-quality DQA sweeps.

This runner keeps the useful 08_3_2/08_3_3 setup, but changes what DQA treats
as pseudo-label quality. Instead of using confidence alone, each client can
write classwise pseudo-label stats based on internal student feature saliency
and local feature contrast around pseudo box centers.

The extra controls here are deliberately about feature-quality:
...
- switch pseudo-label quality mode through DQA0834_STATS_QUALITY_MODE
...
DQA_PROTOCOL_VERSION = "dqa08_3_4_scene_phase2_feature_quality_sweep_v1"
DEFAULT_WORK_ROOT = base.RESEARCH_ROOT / "efficientteacher_dqa08_3_4_phase2_feature_quality_sweep"
...
def _profile() -> str:
    return os.environ.get("DQA0834_PROFILE", "dqa_high_light").strip().lower()


def _source_workspace() -> Path:
    return Path(os.environ.get("DQA0834_SOURCE_WORK_ROOT", str(DEFAULT_SOURCE_WORK_ROOT))).resolve()


def _source_phase1_round() -> int:
    return _int_env("DQA0834_SOURCE_PHASE1_ROUND", DEFAULT_SOURCE_PHASE1_ROUND)
...
def _scheduled(prefix: str, round_idx: int | None, default_start: float, default_end: float | None = None) -> float:
    ...
    total = max(_int_env("DQA0834_PHASE2_ROUNDS", 10), 1)
...
def _seed_phase1_checkpoint(workspace_root: Path) -> Path:
    ...
    force = _bool_env("DQA0834_FORCE_SEED", False)

    if not source.exists():
        raise FileNotFoundError(
            f"Missing source phase1 seed checkpoint: {source}\n"
            "Set DQA0834_SOURCE_WORK_ROOT / DQA0834_SOURCE_PHASE1_ROUND for a different seed."
        )
    ...
    wanted: dict[str, Any] = {
        ...
        "variant": os.environ.get("DQA0834_VARIANT", "unnamed"),
        "profile": _profile(),
        "quality_mode": os.environ.get("DQA0834_STATS_QUALITY_MODE", "feature_balanced"),
    }
    ...
    if target.exists() and not force:
        if metadata_path.exists():
            ...
            if same_seed:
                print(f"Reusing 08_3_4 phase2 seed checkpoint: {target}")
                return target
        raise RuntimeError(
            ...
            "Use DQA0834_FORCE_SEED=1 only if you intentionally want to overwrite it."
        )

    target.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy2(source, target)
    metadata_path.write_text(json.dumps(wanted, indent=2), encoding="utf-8")
    print(f"Seeded 08_3_4 phase2 from DQA08 phase1 round {source_round}: {target}")
    return target
...
def _adjust_decision_for_feature_quality(decision: tri_stage.ThresholdDecision) -> tri_stage.ThresholdDecision:
    if decision.phase is None or decision.phase < 2 or decision.round is None:
        return decision

    low_delta = _scheduled_value("DQA0834_LOW_DELTA", decision.round, 0.0)
    mid_delta = _scheduled_value("DQA0834_MID_DELTA", decision.round, 0.0)
    high_delta = _scheduled_value("DQA0834_HIGH_DELTA", decision.round, 0.0)
    nms_delta = _scheduled_value("DQA0834_NMS_DELTA", decision.round, 0.0)
    min_mid_gap = _float_env("DQA0834_SCHEDULE_MIN_MID_GAP", 0.08)
    min_high_gap = _float_env("DQA0834_SCHEDULE_MIN_HIGH_GAP", 0.12)
    ...
    teacher = _scheduled_value("DQA0834_TEACHER_LOSS_WEIGHT", decision.round, decision.teacher_loss_weight)
    box = _scheduled_value("DQA0834_BOX_LOSS_WEIGHT", decision.round, decision.box_loss_weight)
    obj = _scheduled_value("DQA0834_OBJ_LOSS_WEIGHT", decision.round, decision.obj_loss_weight)
    cls = _scheduled_value("DQA0834_CLS_LOSS_WEIGHT", decision.round, decision.cls_loss_weight)

    return replace(
        decision,
        reason=f"{decision.reason}+feature-quality-scheduled",
...
def _apply_dqa_ssod_profile(cfg: dict[str, Any], *, name: str, target: Path | None) -> None:
    ...
    decision = _adjust_decision_for_feature_quality(tri_stage.decide_thresholds(name, int(cfg["Dataset"]["nc"])))
    ...
    ssod["ignore_obj"] = _bool_env("DQA0834_IGNORE_OBJ", False)
    ssod["pseudo_label_with_obj"] = True
    ssod["pseudo_label_with_bbox"] = _bool_env("DQA0834_PSEUDO_LABEL_WITH_BBOX", True)
    ssod["pseudo_label_with_cls"] = _bool_env("DQA0834_PSEUDO_LABEL_WITH_CLS", True)

    _scheduled_override(ssod, "teacher_loss_weight", "DQA0834_TEACHER_LOSS_WEIGHT", round_idx)
    _scheduled_override(ssod, "box_loss_weight", "DQA0834_BOX_LOSS_WEIGHT", round_idx)
    _scheduled_override(ssod, "obj_loss_weight", "DQA0834_OBJ_LOSS_WEIGHT", round_idx)
    _scheduled_override(ssod, "cls_loss_weight", "DQA0834_CLS_LOSS_WEIGHT", round_idx)
...
def _prepare_phase2_sweep_modules(work_root: Path):
    ...
    original_config = getattr(
        exact_setup,
        "_dqa0834_original_efficientteacher_config",
        exact_setup.efficientteacher_config,
    )
    exact_setup._dqa0834_original_efficientteacher_config = original_config
    ...
    def sweep_config(**kwargs):
        ...
        if target is not None:
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0834_CLIENT_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0834_CLIENT_ORTHOGONAL_WEIGHT", 1e-4)
            ...
            cfg.setdefault("FedSTO", {})["train_scope"] = os.environ.get("DQA0834_SERVER_TRAIN_SCOPE", "all")
            cfg["FedSTO"]["orthogonal_weight"] = _float_env("DQA0834_SERVER_ORTHOGONAL_WEIGHT", 1e-4)
            cfg.setdefault("hyp", {})["lr0"] = _scheduled_value(
                "DQA0834_SERVER_LR0",
                round_idx,
                cfg.get("hyp", {}).get("lr0", 0.01),
...
def _sweep_run_train(fedsto, config: Path, args, *, extra_env: dict[str, str] | None = None):
    ...
    quality_mode = os.environ.get("DQA0834_STATS_QUALITY_MODE", "feature_balanced").strip().lower()
    env["DQA0834_STATS_QUALITY_MODE"] = quality_mode
    env["DQA_STATS_QUALITY_MODE"] = quality_mode
    try:
        import yaml
    ...
    decision = _adjust_decision_for_feature_quality(
    ...
    if decision.phase is not None and decision.phase >= 2 and decision.client_id is not None:
        ...
        env["DQA08_TRI_STAGE_GATE"] = "1" if _bool_env("DQA0834_TRI_STAGE_GATE", True) else "0"
        env["DQA08_IGNORE_THRES_MID"] = json.dumps(decision.ignore_thres_mid)
        env["DQA08_LOW_MID_OBJ_WEIGHT"] = str(
            _scheduled_value("DQA0834_LOW_MID_OBJ_WEIGHT", decision.round, 0.35)
        )
        env["DQA08_MID_HIGH_OBJ_WEIGHT"] = str(
            _scheduled_value("DQA0834_MID_HIGH_OBJ_WEIGHT", decision.round, 0.90)
        )
...
def _dqa_config(args) -> v2.AggregationConfig:
    return v2.AggregationConfig(
        ...
        min_server_alpha=_float_env("DQA0834_MIN_SERVER_ALPHA", 0.65),
        residual_blend=_float_env("DQA0834_RESIDUAL_START", 0.16),
    )
...
def _aggregate_phase2_sweep(
    ...
    residual = _scheduled("DQA0834_RESIDUAL", round_idx, 0.16, 0.08)
    min_server_alpha = _scheduled(
        "DQA0834_MIN_SERVER_ALPHA",
        round_idx,
        _float_env("DQA0834_MIN_SERVER_ALPHA_START", _float_env("DQA0834_MIN_SERVER_ALPHA", 0.65)),
        _float_env("DQA0834_MIN_SERVER_ALPHA_END", _float_env("DQA0834_MIN_SERVER_ALPHA", 0.65)),
    )
    cfg = replace(
        copy.copy(config),
    ...
    state.setdefault("phase2_feature_quality_sweep", []).append(
        {
            "round": round_idx,
            "variant": os.environ.get("DQA0834_VARIANT", "unnamed"),
            "profile": _profile(),
    ...
    print(
        "DQA08_3_4 phase2 gate:",
        f"variant={os.environ.get('DQA0834_VARIANT', 'unnamed')}",
        f"profile={_profile()}",
        f"quality_mode={os.environ.get('DQA0834_STATS_QUALITY_MODE', 'feature_balanced')}",
        f"round={round_idx}",
...
def main() -> None:
    ...
    os.environ["DQA0834_PHASE2_ROUNDS"] = str(parsed.phase2_rounds)
    ...
    os.environ.setdefault(
        "DQA08_THRESHOLD_LOG",
        str((parsed.stats_root / "phase2_feature_quality_policy_schedule.jsonl").resolve()),
    )
    ...
    if parsed.phase1_rounds != 0:
        raise ValueError("08_3_4 is phase2-only. Run with --phase1-rounds 0.")
    ...
    print("DQA08_3_4 profile: phase2-only FedSTO/DQA sweep")
    print(f"DQA08_3_4 variant: {os.environ.get('DQA0834_VARIANT', 'unnamed')}")
    print(f"DQA08_3_4 profile mode: {_profile()}")
    print(f"DQA08_3_4 stats quality mode: {os.environ.get('DQA0834_STATS_QUALITY_MODE', 'feature_balanced')}")
    print(f"DQA08_3_4 seed: {_source_workspace()} phase1 round {_source_phase1_round()}")
    print(f"DQA08_3_4 client train_scope: {os.environ.get('DQA0834_CLIENT_TRAIN_SCOPE', 'all')}")
    print(
        "DQA08_3_4 residual schedule:",
        _float_env("DQA0834_RESIDUAL_START", 0.16),
        "->",
        _float_env("DQA0834_RESIDUAL_END", 0.08),
    )
    print(
        "DQA08_3_4 pseudo loss schedule:",
        _float_env("DQA0834_TEACHER_LOSS_WEIGHT_START", _float_env("DQA0834_TEACHER_LOSS_WEIGHT", 0.34)),
        "->",
        _float_env("DQA0834_TEACHER_LOSS_WEIGHT_END", _float_env("DQA0834_TEACHER_LOSS_WEIGHT", 0.34)),
        "/ box",
        _float_env("DQA0834_BOX_LOSS_WEIGHT_START", _float_env("DQA0834_BOX_LOSS_WEIGHT", 0.02)),
        "->",
        _float_env("DQA0834_BOX_LOSS_WEIGHT_END", _float_env("DQA0834_BOX_LOSS_WEIGHT", 0.02)),
    )
    print(f"DQA08_3_4 dqa_start_phase: {parsed.dqa_start_phase}")
