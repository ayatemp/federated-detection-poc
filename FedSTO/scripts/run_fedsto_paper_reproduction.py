...
import setup_fedsto_paper_reproduction as setup
...
PROTOCOL_VERSION = "fedsto_public_spec_broadcast_reset_ema_v2"
...
def make_start_checkpoint(
    ...
    *,
    ...
    elif reset_ema_to_model and base.get("model") is not None:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--persist-client-ema-across-rounds",
        action="store_true",
        help=(
            "Experimental non-paper variant. By default FedSTO reinitializes each local EMA from the "
            "broadcast global checkpoint after every server broadcast, matching Appendix H.2."
        ),
    )
